...
| --- | --- | --- |
| `chassis.size` | Wheelbase, track, length, width, frontal area, base CG height and **baseMass** (bare steel chassis + body + interior) from `CHASSIS_SIZES`. kei 500 kg, compact 710, mid 880, large 1050 → truck 1450 kg. | Everything scales from here: a bigger car is heavier, drags more air, corners on a longer wheelbase (stable but lazier). |
| `chassis.material` | Chassis mass × factor: steel 1.0, aluminium 0.82, carbon 0.66. | The single biggest lightening lever; carbon removes ~300 kg from a mid chassis. |
...
| Build field | Physical effect | Why you care |
| --- | --- | --- |
| `compound` | All base grip/thermal/wear figures from `TIRE_COMPOUNDS` — see the ladder: street 0.95 → slick_soft 1.45 peak mu, with progressively narrower and hotter temperature windows (asymmetric: `coldGripFloor` below, a gentler `hotGripFloor` 0.70–0.85 above), faster heating (`heatingScale` × `TIRE_HEATING_BASE` = 0.65e-3 °C/J at the reference tyre), slower cooling (`coolingScale` × `TIRE_COOLING_BASE` 0.013 1/s) and faster wear (`wearScale` 0.4 street … 6 soft slick: a soft slick loses ~3 % per 5 laps of clubsprint, a street tyre 0.3 %). Rally/snow tyres carry `surfaceAffinity` > 1 on loose surfaces; drift keeps 95% of grip while sliding. The sim saturates the heating slip speed at 3 m/s and lets loose surfaces absorb `slideRetention` of the slip energy (docs/ASSUMPTIONS.md → Tyres). | The single biggest grip decision — and a *window*, not a number: a slick_soft below 84 °C grips like a street tyre. |
| `width` (mm) | optimalLoad × wr; peakMu × wr^0.08; loadSensitivity × wr^−0.5; underloadPenalty × wr^0.5; cornering/long stiffness × wr^0.3; heating × 1/wr; wear × 1/wr; cooling × wr^0.3 × the compound's `coolingScale` (slicks 0.7–0.8 hold their heat, road tyres 1.0–1.1); mass = 7 + 0.04 × width + 0.6 × rim. | Wider works harder before giving up and lasts longer — but is heavier, slower to warm and lazy when unloaded (a wide rear on a light rear axle is dead rubber). |
| `pressure` (kPa) | optimalLoad × pr^0.6; underloadPenalty × pr^0.7; peakSlipAngle × pr^0.2; heating × pr^0.35; rolling resistance × pr^0.5; stiffness × (p/220)^0.25; peakMu × (1 − 0.12·max(0,(p−260)/60) − 0.10·max(0,(150−p)/30)). | Lower pressure needs more load to work but grips harder when loaded (and runs hotter); higher is crisper and cooler but over ~260 kPa the patch shrinks. |
...
| `rideHeightFront/Rear` (mm) | CG height ± 0.8 × Δride; underbody downforce × ground effect around ref 0.10 m; travel = 0.06 + 0.4 × ride (bump stop 8 × the spring rate beyond 55 % of it, full droop at −45 %; the analysis's `jumpLandingG` = (F₀ + k·travel·(1 + 8 × 0.45))/F₀ uses the same stop); roll centre = 0.3 × ride; drag ± 5%. | Low = less weight transfer + more ground effect + a touch less drag; high = travel for curbs, gravel and jumps-to-be. |
